...
export default function Perfil({ route }) {
  ...
  async function takeAndUploadPhotoAsync() {
    ...
    return await axiosLoggedInConfig()
      .patch(BASE_URL + "user/" + perfil.id + "/", formData)
      .then((resp) => {
        console.log(resp);
        setModalLoading(false);
      })
    ...
    >
      ...
      >
        <Text style={styles.header_text}>
          {" "}
          Bienvenid@, {param_usuario.nombre}
        </Text>
      ...
      {anios < 18 && !perfil.declaracion_jurada && (
        <TouchableOpacity
          onPress={() => {
      ...
      <TouchableOpacity
        onPress={() => {
          Alert.alert(
            "Ups! Estamos trabajando...",
            "Proximamente vas a poder editar tu perfil en segundos ;)"
          );
        }}
        style={{
          flexDirection: "row",
